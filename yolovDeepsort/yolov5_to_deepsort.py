...
#sys.path.append('E:/ActionDetection/AVADataSet2/yolovDeepsort')
sys.path.insert(0, './yolovDeepsort/yolov5/')
sys.path.insert(2, './yolovDeepsort/')
sys.path.insert(1, './yolovDeepsort/deep_sort_pytorch/')
...
def detect(opt):
    ...
    cfg = get_config()
    opt.config_deepsort = './yolovDeepsort/deep_sort_pytorch/configs/deep_sort.yaml'
    cfg.merge_from_file(opt.config_deepsort)
    
    # 这里是avaMin_dense_proposals_train.pkl的路径，
    # 但是之后要使用via标注之后的avaMin_train.csv，因为在微调之后，坐标数量与坐标位置会发生变化
    #anlee  f = open('./mywork/dense_proposals_train_deepsort.pkl','rb')
    f = open('./yolovDeepsort/mywork/dense_proposals_train.pkl','rb')
    ...
    for i in info:
        dets = info[i]
        tempName = i.split(',')
        cfg.DEEPSORT.REID_CKPT = './yolovDeepsort/deep_sort_pytorch/deep_sort/deep/checkpoint/ckpt.t7'
        ...
        tempImg = cv2.imread(source + '/' + tempName[0] + '/' +tempName[0]+'_'+str(int(tempName[1])).zfill(6) + '.jpg') 
        # 获取图片的大小
        imgsz = tempImg.shape
        ...
        '''
        #im0Path = source + '/' + tempName[0] + '/'+ tempName[0] + '_' + str(int(tempName[1])*30+1).zfill(6) + '.jpg'
        im0Path = source + '/' + tempName[0] + '/'+ tempName[0] + '_' + str(int(tempName[1])).zfill(6) + '.jpg'
        im0 = np.array(Image.open(im0Path))
        outputs = deepsort.update(xywhs, confs, clss, im0)
        ...
        if len(outputs) > 0:
            for output in outputs:
                ...
                dicts.append(dict)
        with open('./train_personID_temp.csv',"w") as csvfile: 
